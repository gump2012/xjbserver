/**
 * Created by gump on 4/12/14.
 */

var macroproduct = require("../../../todaylead/todayPublic/urlmacro/macro_cid2_pid21_40");

function creatdb(mongoose){
    var todayCategory = mongoose.model('todayProduct');

    var newCategory = new todayCategory({pid:21
        ,cid:2
        ,title:'EVE G爽润滑油 阴交 肛交用润滑油 一滑到底'
        ,volume:0
        ,recentvolume:0
        ,org_price:18.00
        ,price:9.90
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID21_URL
        ,time:0
        ,gallery:macroproduct.PID21_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:22
        ,cid:2
        ,title:'JOKER外用延时喷剂日本史上最久 最受欢迎'
        ,volume:0
        ,recentvolume:0
        ,org_price:183.00
        ,price:68.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID22_URL
        ,time:0
        ,gallery:macroproduct.PID22_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:23
        ,cid:2
        ,title:'MFPONES男用助勃刺激阴蒂震动环 世界好套环'
        ,volume:0
        ,recentvolume:0
        ,org_price:32.00
        ,price:18.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID23_URL
        ,time:0
        ,gallery:macroproduct.PID23_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:24
        ,cid:2
        ,title:'倍耐力延时喷剂纯植物提取 让男人更从容自信'
        ,volume:0
        ,recentvolume:0
        ,org_price:70.00
        ,price:30.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID24_URL
        ,time:0
        ,gallery:macroproduct.PID24_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:25
        ,cid:2
        ,title:'印度神油印度秘方久战不倒杀菌防病延时金刚'
        ,volume:0
        ,recentvolume:0
        ,org_price:12.00
        ,price:5.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID25_URL
        ,time:0
        ,gallery:macroproduct.PID25_GALLERY});
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:26
        ,cid:2
        ,title:'头牌推荐 百倍安全管理 私密保存'
        ,volume:0
        ,recentvolume:0
        ,org_price:10.00
        ,price:5.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID26_URL
        ,time:0
        ,gallery:macroproduct.PID26_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:27
        ,cid:2
        ,title:'头牌推荐 耐氏男用延时喷剂 超久防早泄 正品防伪'
        ,volume:0
        ,recentvolume:0
        ,org_price:189.00
        ,price:99.0
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID27_URL
        ,time:0
        ,gallery:macroproduct.PID27_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:28
        ,cid:2
        ,title:'延时必备 三色锁精环 根本射不出来'
        ,volume:0
        ,recentvolume:0
        ,org_price:20.00
        ,price:12.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID28_URL
        ,time:0
        ,gallery:macroproduct.PID28_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:29
        ,cid:2
        ,title:'杜蕾斯按摩油 润滑油 让爱更浓烈'
        ,volume:0
        ,recentvolume:0
        ,org_price:108.00
        ,price:35.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID29_URL
        ,time:0
        ,gallery:macroproduct.PID29_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:30
        ,cid:2
        ,title:'正品罪爱费洛蒙香水佛罗蒙调助情兴奋男用女用迷人强效催性欲特价'
        ,volume:0
        ,recentvolume:0
        ,org_price:68.00
        ,price:40.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID30_URL
        ,time:0
        ,gallery:macroproduct.PID30_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:31
        ,cid:2
        ,title:'水晶之恋凤梨套 延时增粗 G点 刺激 宝贝好喜欢'
        ,volume:0
        ,recentvolume:0
        ,org_price:10.00
        ,price:6.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID31_URL
        ,time:0
        ,gallery:macroproduct.PID31_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:32
        ,cid:2
        ,title:'爱妃液人体爱液润滑油 按摩推油 可用肛交'
        ,volume:0
        ,recentvolume:0
        ,org_price:32.00
        ,price:15.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID32_URL
        ,time:0
        ,gallery:macroproduct.PID32_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:33
        ,cid:2
        ,title:'玩爆潮品 12星座延时锁精水晶套 G点阴道刺激异型套'
        ,volume:0
        ,recentvolume:0
        ,org_price:18.00
        ,price:9.90
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID33_URL
        ,time:0
        ,gallery:macroproduct.PID33_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:34
        ,cid:2
        ,title:'玩爆潮品男用龟头增大增粗延时套'
        ,volume:0
        ,recentvolume:0
        ,org_price:28.00
        ,price:16.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID34_URL
        ,time:0
        ,gallery:macroproduct.PID34_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:35
        ,cid:2
        ,title:'男用阴茎增粗增大增长狼牙套 伴君如伴虎'
        ,volume:0
        ,recentvolume:0
        ,org_price:35.00
        ,price:6.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID35_URL
        ,time:0
        ,gallery:macroproduct.PID35_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:36
        ,cid:2
        ,title:'百乐男用阴茎增粗增大增长狼牙套 各种刺激伴君左右'
        ,volume:0
        ,recentvolume:0
        ,org_price:35.00
        ,price:9.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID36_URL
        ,time:0
        ,gallery:macroproduct.PID36_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:37
        ,cid:2
        ,title:'纯天然高级羊眼圈 让您的高潮永驻'
        ,volume:0
        ,recentvolume:0
        ,org_price:16.00
        ,price:6.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID37_URL
        ,time:0
        ,gallery:macroproduct.PID37_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:38
        ,cid:2
        ,title:'美国Enlarge男士阴茎增粗助大精油 外用防早泄'
        ,volume:0
        ,recentvolume:0
        ,org_price:85.00
        ,price:48.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID38_URL
        ,time:0
        ,gallery:macroproduct.PID38_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:39
        ,cid:2
        ,title:'耐氏外用延时湿巾 天然最安全（1片12片'
        ,volume:0
        ,recentvolume:0
        ,org_price:50.00
        ,price:2.50
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID39_URL
        ,time:0
        ,gallery:macroproduct.PID39_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });

    var newCategory = new todayCategory({pid:40
        ,cid:2
        ,title:'菲尔普成人用品玩具专用消毒液 深度清洁 私密防护'
        ,volume:0
        ,recentvolume:0
        ,org_price:56.00
        ,price:14.00
        ,state:1
        ,stamper:''
        ,pic_url:macroproduct.PID40_URL
        ,time:0
        ,gallery:macroproduct.PID40_GALLERY
    });
    newCategory.save( function( err, silence ) {
        if( err )
        {
            console.log(err);
        }
    });
}

exports.createdb = creatdb;